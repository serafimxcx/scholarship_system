$(function(){
    ...
    $(document.body).on('click', '.btn_viewinfo', function(){
        ...
        window.location.href= "scholars.php?application_id="+application_id+"&view_info=true";
    });
    ...
    $("#btn_accept").click(function(){
        ...
        if(confirm("Are you sure you want to disqualify this scholar? ")){
            if($("#txt_reason").val() == ""){
                $(".message_div").text("Please state the reason.");
    ...
    $("#slct_appstatus").change(function(){
        ...
        if(status != "Pending"){
            if(confirm(`Are you sure you want to update the status of this scholar as ${status}?`)){
...
function printRecords(application_id){
    var cParam="";
    cParam = "name="+$("#txt_name").val();
    cParam += "&slct_yearsem="+$("#slct_yearsem").val();
    cParam += "&slct_program="+$("#slct_program").val();
    ...
    var cFile = "print_scholars.php?"+cParam;
