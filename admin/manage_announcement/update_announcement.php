    ...
    if(!empty($_FILES["slct_image"]["name"])){
        ...
        if(in_array($fileType, $allowTypes)){
            ...
            if(move_uploaded_file($_FILES["slct_image"]["tmp_name"], $targetFilePath)){
                ...
                $values= "title='$title', body='$body', picture='$image', updated_at='$updated_at'";
        ...
        $values= "title='$title', body='$body', picture='', updated_at='$updated_at'";

        
    }

    $insert = $conn->query("update tb_announcement set $values where id='$_POST[txt_announcement_id]'");


    if($insert){
        $response = array('success'=>true, 'message'=>"Announcement Updated Successfully.");
        echo json_encode($response);
    }else{
